  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found') {
        alert(t('auth.errors.noAccount'));
      } else if (error.code === 'auth/wrong-password') {
        alert(t('auth.errors.wrongPassword'));
      } else if (error.code === 'auth/invalid-credential') {
        alert(t('auth.errors.invalidCredential'));
      } else {
        alert(t('auth.errors.generic') + error.message);
      }
    }
  };
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        alert(t('auth.errors.emailInUse'));
      } else if (error.code === 'auth/invalid-email') {
        alert(t('auth.errors.invalidEmail'));
      } else {
        alert(t('auth.errors.generic') + error.message);
      }
    }
    ...
    <div className={`auth-overlay ${isOpen ? 'active' : ''}`} onClick={handleOverlayClick}>
      ...
      <div className="sign-in-form" style={{ display: showSignUp ? 'none' : 'flex' }}>
        ...
        <div className="form1">
          ...
          <form onSubmit={handleSignIn}>
            <div className="email">
              ...
              />
              <p className="required2" style={{ display: signInErrors.email ? 'block' : 'none' }}>{t('auth.required')}</p>
            </div>
            ...
            <div className="password">
              ...
              ></i>
              <p className="required1" style={{ display: signInErrors.password ? 'block' : 'none' }}>{t('auth.required')}</p>
              <a href="#">{t('auth.forgotPassword')}</a>
            </div>
          </form>

          <button className="sign-in" onClick={handleSignIn}>{t('auth.signIn')}</button>
        ...
        <div className="sign-up-link">
          <h1 className="head2">{t('auth.orBecomeMember')}</h1>
          <p className="join">{t('auth.joinText')} <span>Magical Africa</span> {t('auth.joinTextEnd')}</p>
          <button className="sign-up-btn" onClick={() => setShowSignUp(true)}>{t('auth.signUp')}</button>
        </div>
      </div>
      ...
      <div className="sign-up-form" style={{ display: showSignUp ? 'flex' : 'none' }}>
        <div className="side1">
          <h1>{t('auth.journeyStarts')}</h1>

          <div className="cards">
            <div className="card1">
              <i className="fa-solid fa-globe-africa"></i>
              <div>
                <h2>{t('auth.cards.explore.title')}</h2>
                <p>{t('auth.cards.explore.description')}</p>
              </div>
            </div>
            ...
            <div className="card2">
              ...
              <div>
                <h2>{t('auth.cards.connect.title')}</h2>
                <p>{t('auth.cards.connect.description')}</p>
              </div>
            </div>
            ...
            <div className="card3">
              ...
              <div>
                <h2>{t('auth.cards.travel.title')}</h2>
                <p>{t('auth.cards.travel.description')}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="side2">
          ...
          <form onSubmit={handleSignUp}>
            <div className="tribe-div">
              <label><span className="color">*</span> {t('auth.tribes')}</label>
              <select 
            ...
            <div className="names-div">
              <div className="firstname">
                <label><span className="color">* </span>{t('auth.firstName')}</label>
                <input 
                  type="text" 
                ...
                />
                <p className="required3" style={{ display: signUpErrors.firstName ? 'block' : 'none' }}>{t('auth.required')}</p>
              </div>

              <div className="secondname">
                <label><span className="color">* </span>{t('auth.lastName')}</label>
                <input 
                  type="text" 
                ...
                />
                <p className="required4" style={{ display: signUpErrors.lastName ? 'block' : 'none' }}>{t('auth.required')}</p>
              </div>
            </div>

            <div className="email-div">
              <label><span className="color">*</span> {t('auth.email')}</label>
              <input 
                type="text" 
              ...
              />
              <p className="required5" style={{ display: signUpErrors.email ? 'block' : 'none' }}>{t('auth.required')}</p>
            </div>

            <div className="where-div">
              <div className="password2">
                <label><span className="color">* </span>{t('auth.password')}</label>
                <input 
                ...
                ></i>
                <p className="required6" style={{ display: signUpErrors.password ? 'block' : 'none' }}>{t('auth.required')}</p>
              </div>

              <div className="country2">
                <label><span className="color">* </span>{t('auth.country')}</label>
                <input 
                  type="text" 
                ...
                />
                <p className="required7" style={{ display: signUpErrors.country ? 'block' : 'none' }}>{t('auth.required')}</p>
              </div>
            </div>

            <button type="submit" className="create-account">{t('auth.createAccount')}</button>
          </form>
        </div>
      </div>
    </div>
