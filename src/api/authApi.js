...
export const signup = async ({ firstName, secondName, email, password, gender, telephone }) => {
  ...
  const res = await api.post('/api/v1/auth/generate', payload);
  return { ...res.data, username };
};


export const login = async (userName, password) => {
  const res = await api.post('/api/v1/auth/authenticate', { userName, password });
  return res.data;
};


export const refreshAuthToken = async (token) => {
  const res = await api.post('/api/v1/auth/refreshToken', { token });
  return res.data;
};
